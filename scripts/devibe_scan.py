...
RULES = [
    ...
    },
    {
        ...
        "pats": [
            r"[\U0001F680✨⚡\U0001F525\U0001F4A1\U0001F512✅\U0001F3AF\U0001F31F\U0001F6E1\U0001F4C8\U0001F511\U0001F389\U0001F680]"
        ],
    },
    {
        "id": "generic-font",
    ...
    # ---- LOW: copy + minor ----
    {
        ...
        "pats": [
            ...
            r"\bEffortlessly\b",
            r"\breimagined\b",
            r"take your [^.]{0,30}to the next level",
            r"\bGame-?changer\b",
        ],
    },
    {
...
def compile_rules(min_sev):
    ...
    for r in RULES:
        ...
        r["suppress_rx"] = (
            re.compile(r["suppress"], re.IGNORECASE) if r.get("suppress") else None
        )
        out.append(r)
    return out
...
def main():
    ap = argparse.ArgumentParser(
        description="Scan a web codebase for vibe-coded design tells."
    )
    ...
    ap.add_argument(
        "--max", type=int, default=10, help="max examples shown per rule (text mode)"
    )
    args = ap.parse_args()
            ...
            json.dumps(
                {
                    ...
                    "counts": by_sev,
                    "vibe_score": weighted,
                    "verdict": verdict(by_sev, weighted),
                    "findings": findings,
                },
                indent=2,
            )
    ...
    rule_ids = sorted(
        by_rule, key=lambda rid: (sev_order[by_rule[rid][0]["sev"]], -len(by_rule[rid]))
    )
    print(f"\n  unslop-ui scan: {args.path}")
    print(
        f"  files scanned: {files_scanned}   findings: {len(findings)}   vibe score: {weighted}"
    )
    ...
    if not findings:
        print(
            "  Nothing flagged. Either it is clean or the tells are layout/motion ones a regex"
    ...
    for rid in rule_ids:
        ...
        tag = f0["sev"].upper()
        print(
            f"  [{tag}] {f0['label']}  ({len(items)} hit{'s' if len(items)!=1 else ''})"
        )
        print(f"        fix: {f0['fix']}")
