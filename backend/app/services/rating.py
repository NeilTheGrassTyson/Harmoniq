...
def _can_view(
    ...
    if viewer_id is not None and viewer_id == rating.user_id:
        return True
    scope = VisibilityScope(rating.visibility)
    if scope == VisibilityScope.PUBLIC:
        return True
    if scope == VisibilityScope.FRIENDS and viewer_is_friend:
        return True
    return False
...
async def get_aggregate(
    ...
    """Average of each user's most recent score for the given entity."""
    rn = (
        func.row_number()
        .over(
...
async def list_for_entity(
    ...
    for rating, user in rows:
        is_friend = (
            await _is_friend(session, viewer_id, rating.user_id)
            if viewer_id is not None and viewer_id != rating.user_id
            else False
        )
        if not _can_view(rating, viewer_id, is_friend):
            continue
        visible.append(
            RatingRead(
                id=rating.id,
...
async def list_for_user(
    ...
    is_friend = (
        await _is_friend(session, viewer_id, profile_user_id)
    ...
    rows_result = await session.execute(
        select(Rating)
        .where(Rating.user_id == profile_user_id)
        .order_by(Rating.created_at.desc(), Rating.id.asc())
    )
    ratings = rows_result.scalars().all()

    visible_ratings = [r for r in ratings if _can_view(r, viewer_id, is_friend)]
...
async def count_for_user(
    ...
    rows_result = await session.execute(select(Rating).where(Rating.user_id == user_id))
    ratings = rows_result.scalars().all()
    is_friend = (
        await _is_friend(session, viewer_id, user_id)
        if viewer_id is not None and viewer_id != user_id
        else False
    )
    return sum(1 for r in ratings if _can_view(r, viewer_id, is_friend))
