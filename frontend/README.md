...
│   │   ├── layout.tsx              Root layout — ClerkProvider, global nav with SearchBar
│   │   ├── page.tsx                Home route (/)
│   │   ├── globals.css             Global styles + Tailwind import
│   │   ├── artist/[mbid]/page.tsx  Artist detail page (public)
│   │   ├── album/[mbid]/page.tsx   Album detail page (public)
│   │   └── track/[mbid]/page.tsx   Track detail page (public)
│   ├── components/
│   │   ├── SearchBar.tsx           Debounced live search — grouped Artist/Album/Track results
│   │   └── CoverArt.tsx            Cover art image with CAA 404 fallback placeholder
│   ├── lib/
│   │   ├── api.ts                  Fetch wrapper — attaches Clerk token to requests
│   │   └── catalog.ts              Typed client for catalog endpoints (no auth required)
...
Clerk handles the session UI. The middleware (`src/middleware.ts`) protects all routes except `/`, `/sign-in`, `/sign-up`, and the public catalog routes (`/artist/*`, `/album/*`, `/track/*`).
